/**
   Current implementation looks like some FS for Windows.
   But it isn't. Please fix every line which has
   a comment that contains ``hack''.

   NOTE: current version hangs when huge file is written and free
   memory is low. Looks like problem is in dirty page preemption.
 */
